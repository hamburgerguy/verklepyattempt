...
class VerkleTree:
    ...
    def hashAllCurrNodes(self, nodes: list) -> list:
        return [hash_to_scalar('verkle', node) for node in nodes]
    ...
    def root(self, printAllCommit=False) -> Point:
        veclen = 2 ** self.exponent   # vector/polynomial length
        currnodes = self.datablocks
        self.G_vec = PointVector([random_point() for i in range(veclen)])   # (public)
        while len(currnodes) > 1:
            ...
            for i in range(0, len(nodehashes), veclen):
                ...
                P = poly ** self.G_vec + r * H   # the actual commitment
                cache1.append(P)
    ...
    def open(self, index: int) -> tuple:
        ...
        for i in range(self.exp1):
            ...
            x = Scalar(curridx + 1)
            v = hash_to_scalar('verkle', currpathdata)
...
def check(index: int, datum: object, proofs: list, root: Point) -> bool:
    ...
    if proofs[0]['state'][2] != hash_to_scalar('verkle', datum) or proofs[-1]['state'][0] != root:
        return False
    for proof in proofs:
        #  verify the proofs
        if not verify(proof):
